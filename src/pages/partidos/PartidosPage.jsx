...
import { eliminarPartido, partidosEquipoTorneo } from "../../api/partidosApi";
...
export function PartidosPage() {
  ...
  const busquedaPartidos = (idEquipo, idTorneo) => {
    ...
    partidosEquipoTorneo(idEquipo, idTorneo)
      .then(
  ...
  //Llamada a la api para eliminar el partido, que vamos a pasar como prop a la card del partido
  const eliminar = (id) => {
    //Llamamos al cliente apiFecth para eliminar el partido con el id recibido
    eliminarPartido(id)
      .then(
        //Actualizamos los partidos excluyendo el partido con el id recibido
        () => setPartidos(prevPartidos => prevPartidos.filter((p) => p.id !== id))
      )
      .catch((error) => console.error(error));
  };
    ...
    <>
      ...
      {partidos.length === 0 ? (
        ...
        partidos.map((partido) => (
          <CardPartido partido={partido} eliminar={eliminar} key={partido.id} />
        ))
      )}
